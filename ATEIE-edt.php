		...
		<tr>
			<td colspan="2" class="titulocorpo">
					<table class="titulocorpo_tabela" border="0">
						<tr>
							<td align="center"><b>ATEIE - Autorização para Trabalhos em Equipamentos de Interligação Energizados</b></td>
						</tr>
					</table>
			</td>
		</tr>
...
<div align="center" style="width: 100%; height: 100%;">
				...
				<div id="toolbar1" style="padding:5px;">
					...
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editarATEIE()">Editar</a><font size="3" color='grey'> | </font>
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-redo" plain="true" onclick="criaridenticoATEIE()">Criar Idêntica</a><font size="3" color='grey'> | </font>
		...
		function imprimir(){
			...
			window.open("EDT/gerarpdf/index_ATEIE.php?id_ateie="+row.id_ateie);
			...
			if (row){
				$.messager.confirm('Confirmação','Tem certeza que deseja remover o ATEIE?',function(r){
					if (r){
						$.post('EDT/destroy_ATEIE.php',{id:row.id_ateie},function(result){
							if (result.success){
								$('#dg3').datagrid('reload');	// reload the user data
								$.messager.show({title:'Mensagem',msg:'ATEIE Removido com Sucesso',timeout:2500,showType:'slide'});
							} else {
		...
		function editarATEIE(){
			var row = $('#dg3').datagrid('getSelected');
			if (row){
				window.location.href='ATEIE-edita-form.php?name=<?php echo $name;?>&id_ateie='+row.id_ateie;
			}
		}


		function criaridenticoATEIE(){
			var row = $('#dg3').datagrid('getSelected');
			if (row){
				window.location.href='ATEIE-duplica-form.php?name=<?php echo $name;?>&id_ateie='+row.id_ateie;
			}
		}
...
<!--Diálogo para adicionar nova ATEIE-->
					...
					<table border="0" style="width:100%;">
						...
						<td>
							<div style="margin-bottom:10px">
								<input class="easyui-textbox" prompt="Equipamento sob Intervenção" name="equipamento" iconWidth="28" style="width:370px;height:34px;padding:15px;" data-options="prompt:'Username',iconCls:'',iconWidth:38" required="true">
							</div>
							<div style="margin-bottom:10px">	
							...
							<div style="margin-bottom:35px">
								Condições Requeridas:<br><small><small><small><br></small></small></small>
	...
	<script>	
			...
			$('#fm6').form('submit',{
				...
				success: function(result){
						...
						$('#dg3').datagrid('reload');	// reload the user data
						$.messager.show({title:'Mensagem',msg:'ATEIE Criado com Sucesso',timeout:2500,showType:'show'});
				}
			});
		}
	</script>
<!-- FIM Diálogo para adicionar nova ATEIE-->
...
include "Mensagens/msg_ATEIE.php";
 ?>
